...
public class ProdutoService : IProdutoService
{
    ...
    public async Task<List<Produto>> GetListaValorCresceste() =>
        (await GetListaProdutos()).OrderBy(x => x.Valor).ToList();

        
    public async Task<List<Produto>> GetListaValorDecresceste() =>
        (await GetListaProdutos()).OrderByDescending(x => x.Valor).ToList();    
